...
BTrack::BTrack(int hopSize_, int frameSize_)
    : odf(hopSize_, frameSize_, ComplexSpectralDifferenceHWR, HanningWindow),
      ...
      tempoObservationVector(41),
      delta(41),
      prevDelta(41),
      prevDeltaFixed(41),
      tempoTransitionMatrix(41, std::vector<double>(41)) {
...
void BTrack::resampleOnsetDetectionFunction() {
  for (int i = 0; i < std::min(512,onsetDFBufferSize); i++) {
    resampledOnsetDF[i] = (float)onsetDF[i];
  }
  return;
/*
  float output[512];
...
//=======================================================================
double BTrack::calculateMeanOfArray(const std::vector<double>& array,
...
//=======================================================================
void BTrack::normaliseArray(std::vector<double>& array, int N) {
  double sum = 0;
